...
samples = []
directories = os.listdir('sim_images')
for dir in directories:
    samples+=['/'+dir+'/'+x for x in os.listdir('sim_images/'+dir)]
    ...
    while 1: # Loop forever so the generator never terminates
        ...
        for offset in range(0, num_samples, batch_size):
            ...
            for name in batch_samples:
                image = cv2.imread('sim_images/'+name)
...
def Preprocessing():
    ...
    model.add(Cropping2D(cropping=((0,150),(0,0))))
...
validation_data=validation_generator, validation_steps=len(validation_samples)/8, epochs=5, verbose = 1)

model.save("TL_sim_classifier.h5")
model.save_weights("TL_sim_classifier_weights.h5")
